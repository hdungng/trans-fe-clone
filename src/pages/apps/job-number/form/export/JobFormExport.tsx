...
import { ExtractResponse, GeneralFormProps, ExportProduct, ExportExtractResponse } from 'types/pages/form-field';
...
import ExtractTab1 from '../../tabs/export/ExtractTab1';
import ExtractTab2 from '../../tabs/export/ExtractTab2';
...
import { containerControlExport, generalFormFieldsControlExport, validationFormExport } from 'types/extract-form-field/extract-export-form';
import { markAllFieldsTouched } from 'types/extract-form-field/extract-import-form';
import ProductEditFormDialog from './ProductEditForm';
import comboBoxData, { getLabelByCode } from 'data/comboBoxInfo';
import { initialExportFormFactory } from '../formik/ExportExtractFormik';
import { APIResponse } from 'types/response';
import { Box } from '@mui/material';
import { useIntl } from 'react-intl';

const JobFormExport = forwardRef(({ initialFormValues, setInitialFormData, jobNumberId, initStepData, setIsECUSLoading }: GeneralFormProps, ref) => {


    const validationSchema = validationFormExport(yup);
    const generalFormFields1 = generalFormFieldsControlExport;
    const generalFormFields2 = containerControlExport;
    ...
    const [editingProduct, setEditingProduct] = useState<ExportProduct | undefined>(undefined);
    const [productData, setProductData] = useState<ExportProduct[]>([]);
    ...
    const formikValues = initialExportFormFactory(initialFormValues as ExportExtractResponse, validationSchema, setInitialFormData)
    ...
    const tempoSave = async () => {
        ...
        const values = formik.values as ExportExtractResponse;

        const {
            final_delivery_location,
            final_delivery_location_name,
            loading_location_code,
            loading_location_code_name,
            ...restGeneralInfo
        } = values.general_info;

        // Tạo general_info mới với điều kiện kiểm tra 'VNZZZ'
        const general_info = {
            ...restGeneralInfo,
            final_delivery_location,
            loading_location_code,
            ...(final_delivery_location === 'VNZZZ' && { final_delivery_location_name }),
            ...(loading_location_code === 'VNZZZ' && { loading_location_code_name }),
        };
        ...
        const response: APIResponse = await updateExtract(jobNumberId, initStepData.method, payload);
    ...
    const handleEditClick = (product: ExportProduct) => {
                    ...
                    ({ values, setFieldValue }) => (
                        <>
                            ...
                            <Tabs value={tabValue} onChange={handleChange} sx={{ mb: 5 }}>
                                <Tab label={intl.formatMessage({ id: 'job-number.detail.extract.export.tabs.general', defaultMessage: 'General information' })} value={1} />
                                <Tab label={intl.formatMessage({ id: 'job-number.detail.extract.export.tabs.container', defaultMessage: 'Container information' })} value={2} />
                                <Tab label={intl.formatMessage({ id: 'job-number.detail.extract.export.tabs.products', defaultMessage: 'Product list' })} value={3} />
                            </Tabs>
                            {/* 1 */}
                            {tabValue === 1 && (
                                <ExtractTab1 generalFormFields1={generalFormFields1} />
                            )}
                            {tabValue === 2 && (
                                <ExtractTab2 generalFormFields2={generalFormFields2} />
                            )}
