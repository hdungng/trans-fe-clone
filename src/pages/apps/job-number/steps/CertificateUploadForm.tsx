    ...
    Yup.object({
        files: Yup.array()
            .of(
                Yup.mixed<File>().test(
                    'fileType',
                    intl.formatMessage({ id: 'job-number.edit.upload.validation.file-type', defaultMessage: 'Files must be PDF, Excel, or CSV' }),
                    (value) =>
                        value &&
                        (value.type === 'application/pdf' ||
                            ...
                            value.type === 'text/csv' ||
                            value.type === 'application/vnd.ms-excel.sheet.binary.macroEnabled.12')
                )
            )
            .required(intl.formatMessage({ id: 'job-number.edit.upload.validation.required', defaultMessage: 'Supporting documents are required' })),
    });
