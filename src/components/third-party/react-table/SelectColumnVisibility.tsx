      ...
      <Select
        ...
        renderValue={() => {
          if (getIsAllColumnsVisible()) {
            return <Typography variant="subtitle1">Tất cả các cột đều hiển thị</Typography>;
          }

          return <Typography variant="subtitle1">{getVisibleLeafColumns().length} cột đang hiển thị</Typography>;
        }}
        MenuProps={MenuProps}
        size="small"
      >
        <MenuItem value="all" onClick={getToggleAllColumnsVisibilityHandler()}>
          ...
          <ListItemText primary="Tất cả cột" />
        </MenuItem>
        {getAllColumns().map(
          (column) =>
