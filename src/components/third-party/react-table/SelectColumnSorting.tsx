  ...
  return (
    <FormControl sx={{ width: 200 }}>
      <Select
        ...
        input={<OutlinedInput id="select-column-sorting" placeholder="chọn cột" />}
        renderValue={(selected) => {
          const selectedColumn = getAllColumns().filter((column) => selected.length > 0 && column.id === selected[0].id)[0];
          if (selectedColumn) {
            return (
              <Typography variant="subtitle2">
                Lọc theo ({typeof selectedColumn.columnDef.header === 'string' ? selectedColumn.columnDef.header : '#'})
              </Typography>
            );
          }
          return <Typography variant="subtitle2">Lọc theo</Typography>;
        }}
        MenuProps={MenuProps}
        size={size}
      >
