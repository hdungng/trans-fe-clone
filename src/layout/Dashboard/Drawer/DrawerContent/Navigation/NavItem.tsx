...
export default function NavItem({ item, level, isParents = false, setSelectedID }: Props) {
  ...
  if (item.id === "users" && !hasPermission(['Admin', 'Manager'])) return;
  if (item.id === "dashboard" && !hasPermission(['Admin', 'Manager'])) return;
  if (item.id === "user-access-control" && !hasPermission(['Admin'])) return;
